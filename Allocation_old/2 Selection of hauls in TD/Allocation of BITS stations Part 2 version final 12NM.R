...
# 1.  Update the TD call to most reasent TD version and include/exclude Hauls in Russian EEZ
# 2.  RUN FIRST STEP TO LINE 205
...
# 4.  RUN SECOND STEP FROM LINE 206 AND OUT.
...
NHaulPlanned <- read.table(paste0(wd, "1 Distribution of stations on strata/Output files/NHaulPlanned.csv"), header=TRUE,sep=";")
NHaulPlanned <- NHaulPlanned [NHaulPlanned$NHauls>0,]
#TD data  from all Sub-Division is read
TD <- read.xlsx(paste0(wd, "2 Selection of hauls in TD/Input data/TD_v8.xlsx"),
                sheet = "all SD")
TD <-TD[TD$Status=="V",]
TD <-TD [,c("NrHaul", "RectangleAlpha", "ICES.SD", "Latitude1_deg", "Latitude1_dec_min", "Longitude1_deg", "Longitude1_dec_min", "Layer", "EEZ", "12NM")] 


#Hauls in Russian EEZ areexcluded from TD because Russia is not participating and no one else can enter Russian EEZ
TD <- subset(TD, EEZ != "RUS")
#TD <- subset(TD, EEZ != "EST")
...
TD <-TD [,c("NrHaul", "RectangleAlpha", "ICES.SD", "Latitude1_deg", "Latitude1_dec_min", "Longitude1_deg", "Longitude1_dec_min", "Layer", "EEZ", "12NM", "Rec9")]
...
write.table(Selectedhauls, paste0(wd, "2 Selection of hauls in TD/Output data/list of allocated hauls.csv"), sep=";")
...
#   Do not change unless you want another substitution pattern

#ModAdditionalSelect$Layer[ModAdditionalSelect$cond==224] <- 2
ModAdditionalSelect$Layer[ModAdditionalSelect$cond==244] <- 2
