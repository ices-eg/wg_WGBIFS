...
# 3.  ADJUST THE STRATA FOR THE ADDITIONAL SELECTION BY USE OF "STRATA ADJUSTMENTS" (step 2) BASED ON COMMENTS IN LOG
...
TD<-read.table("H:/Active nonsystem/ICES WG/WGBIFS/Surveys/Allocating of BITS stations/Trawl database/Database/TD_updated 2.csv",header=TRUE,sep=";")
TD <-TD[TD$Status=="V",]
...
#ModAdditionalSelect$Layer[ModAdditionalSelect$cond==223] <- 2
#ModAdditionalSelect$Layer[ModAdditionalSelect$cond==244] <- 3
ModAdditionalSelect$Layer[ModAdditionalSelect$cond==252] <- 3
#ModAdditionalSelect$Layer[ModAdditionalSelect$cond==275] <- 4
